...
if [[ ! -a $POSTFIX_BASE/postfix-ldap-aliases.cf ]]; then
	...
	sudo postconf -e "virtual_alias_maps=ldap:$POSTFIX_BASE/postfix-ldap-aliases.cf"
fi
...
if [[ ! -a $POSTFIX_BASE/postfix-ldap-local-recipients.cf ]]; then
	...
	sudo postconf -e "local_recipient_maps=ldap:$POSTFIX_BASE/postfix-ldap-local-recipients.cf"
fi
# The Postfix password must be updated, because it was updated in the LDAP
# entry as well.
...
if [[ ! -a $POSTFIX_BASE/postfix-ldap-canonical-map.cf ]]; then
	...
	sudo postconf -e "canonical_maps = ldap:$POSTFIX_BASE/postfix-ldap-canonical-map.cf"
